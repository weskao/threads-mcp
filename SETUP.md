...
> [!TIP]
> **💡 如何使用 `npm run customize-docs` 一鍵更新文件連結？**
>
> 1. 前往 [Meta for Developers 我的應用程式](https://developers.facebook.com/apps/)，點擊您建立的 Threads 應用程式。
> 2. **直接複製瀏覽器網址列的完整網址**貼入終端機，無需手動查找 ID。支援格式：
...
> 3. 腳本自動從網址解析 App ID 與 Business ID（選用），以明碼寫入 `.env`（欄位 `APP_ID` / `BUSINESS_ID`，與 `THREADS_APP_ID` 是不同欄位），並替換文件中的連結預留位置。
>
> 請在終端機執行：
>
> ```bash
...
> ```
>
> *(您也可以選擇使用編輯器的「搜尋與取代」功能手動替換文件中的 `<your_app_id>` 與 `<your_business_id>`)*
