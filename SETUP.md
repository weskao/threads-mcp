...
5. 腳本會自動呼叫 Threads API 交換 60 天長期 Token，並寫入 `.env` 檔案（在 macOS 會詢問是否存入系統 Keychain）。
...
2. 讀取 Token 來源：
   * **macOS**：優先從系統 Keychain 取得（使用 `sh -c` 動態讀取，Token 不會明文寫入設定檔）
   * **其他平台**：從 `.env` 讀取明文 Token
