...
2. **Meta 開發者帳號**：請前往 [Meta for Developers](https://developers.facebook.com/apps/) 登入並完成開發者身分註冊。
...
安裝完成後，進入第一階段，在 Meta 開發者後台建立應用程式（瀏覽器操作）。
...
1. 登入 [Meta for Developers](https://developers.facebook.com/apps/)。
2. 點擊右上角的 **「我的應用程式」 (My Apps)**，然後點擊 **「建立應用程式」 (Create App)**。
3. 選擇 **「其他」 (Other)**，點擊繼續。
4. 類型選擇 **「商業」 (Business)** 或符合您用途的類型，填寫應用程式名稱後建立。

> [!TIP]
> **💡 建立完應用程式後，可用 `npm run customize-docs` 一鍵將 App ID 填入文件連結**
>
> 在瀏覽器網址列複製您剛建立的應用程式網址，直接貼入終端機即可——無需手動查找 ID。支援格式：
>
> - 只有 App ID：`https://developers.facebook.com/apps/123456789`
> - 含 Business ID：`https://developers.facebook.com/apps/123456789/dashboard/?business_id=987654321`
>
> 腳本自動從網址解析 App ID 與 Business ID（選用），寫入 `.env` 並產生 **`SETUP.local.md`**——這份文件中的連結已填入您的 App ID，後續步驟請改看這份文件。
>
> ```bash
> npm run customize-docs
> ```
>
> 執行後開啟 **`SETUP.local.md`** 繼續操作（此文件已加入 `.gitignore`，不會被 git 追蹤）。
>
> *(也可用編輯器的「搜尋與取代」手動替換文件中的 `<your_app_id>` 與 `<your_business_id>`)*

### 2. 啟用 Threads API 使用案例 (Use Cases)

1. 前往應用程式的 [Use cases (使用案例) 設定頁面](https://developers.facebook.com/apps/<your_app_id>/use_cases/customize/settings/?use_case_enum=THREADS_API&selected_tab=settings&product_route=threads-api)。
...
4. **接受測試人員邀請（必做）**，可選擇以下任一方式：

   **方式 A：手機端（Threads App）**
   - 開啟手機上的 **Threads App**。
   - 前往個人檔案頁面右上角的選單，依序選擇 **「設定」 > 「帳號」 > 「網站權限」 (Settings > Account > Website Permissions)**。
   - 點擊 **「測試人員邀請」** 標籤，再點選 **「接受」**。

   **方式 B：電腦端（瀏覽器）**
   - 以瀏覽器開啟 [Threads 網頁版](https://www.threads.com)，並登入您的帳號。
   - 點擊左側選單底部的 **「更多設定」 (More settings)**。
   - 進入 **「網站權限」 (Website Permissions)**，切換到 **「邀請」 (Invites)** 標籤。
   - 找到邀請通知後，點擊 **「Accept」** 完成接受。

   ![Threads 電腦端接受測試人員邀請 — Website Permissions > Invites > Accept](/docs/images/threads-desktop-accept-tester-invite.png)
...
1. 在 Meta 開發者主控台的左側選單中，點擊 **「Use cases」 (使用案例)**，在 **「Access the Threads API」** 右側點擊 **「Customize」 (自訂)**，接著切換至頂部的 **「Settings」 (設定)** 頁籤。
   - 或者直接進入 [Threads API 設定頁面](https://developers.facebook.com/apps/<your_app_id>/use_cases/customize?use_case_enum=THREADS_API&selected_tab=settings&product_route=threads-api)
...
執行上述任一方案的腳本時，若偵測到系統安全金鑰庫（macOS Keychain、Windows PasswordVault、Linux Secret Service），腳本會自動詢問是否加密儲存憑證與 Token——以下說明此流程的細節，**不需要提前手動操作**。

### 1. App ID 與 App Secret 的金鑰庫整合

當您執行 `npm run get-token` 或 `npm run exchange-token` 時，若系統安全金鑰庫或 `.env` 檔案中皆未設定憑證，腳本會主動提示您手動輸入，並進行以下自動化流程：

1. **自動偵測**：優先嘗試從系統安全金鑰庫中取得憑證，若未找到，則備用讀取 `.env` 中的環境變數（支援明文或動態解密指令），若皆無則提示手動輸入。
2. **安全儲存詢問**：若系統安全金鑰庫中尚未儲存憑證，腳本會主動詢問是否將其存入系統安全儲存區。
3. **自動改寫與寫回**：
...
- **若選擇 `y`**：腳本會將長期 Token 安全存入系統金鑰庫，並自動將 `.env` 檔案中的 `THREADS_ACCESS_TOKEN` 值替換為安全的讀取指令：
